...
class MomentService {
  ...
  async getMomentById(momentId) {
    const statement = `
    SELECT m.id id, m.content cotent, m.createAt createTime, m.updateAt updateTime,
        JSON_OBJECT('name', u.name, 'id', u.id, 'avatarUrl', u.avatar_url) author,
        IF(COUNT(l.id),		JSON_ARRAYAGG(JSON_OBJECT('id', l.id, 'name', l.name)) ,NULL) labels,
        (SELECT IF(COUNT(c.id),JSON_ARRAYAGG(
        JSON_OBJECT('id', c.id, 'content', c.content, 'commentId', c.comment_id, 'createTime', c.createAt,
                'user', JSON_OBJECT('id', cu.id, 'name', cu.name, 'avatarUrl', cu.avatar_url))), NULL) FROM comment c LEFT JOIN user cu ON c.user_id = cu.id WHERE m.id = c.moment_id ) comments,
...
`;
  ...
  async getMomentList(offset, size) {
    const statement = `
    SELECT m.id id, m.content cotent, m.createAt createTime, m.updateAt updateTime,
        JSON_OBJECT('name', u.name, 'id', u.id) author,
